...
// ListDirectory lists files inside SMB container relative path
func ListDirectory(ctx context.Context, containerId string, relPath string, maxEntries int) ([]docker.FileEntry, bool, error) {
	entries, truncated, err := docker.ListSmbDir(ctx, containerId, relPath, maxEntries)
	if err != nil {
...
// UploadFileToContainer copies a local file into the container's SMB share (/shares)
func UploadFileToContainer(ctx context.Context, containerId string, destRelPath string, localPath string) error {
	...
	destination := filepath.Join("/shares", destRelPath)
